...
impl LinePoly {
    ...
    pub fn from_line(mut line: impl Iterator<Item = Vec3>, w: f32) -> Self {
        ...
        if let (Some(p0), Some(p1)) = (p0, p1) {
            ...
            let mut dir = (p1 - p0).truncate().normalize_or_zero();
            let mut flip = false;
            let n0 = Self::vertical(dir);
            vertices.push(p0 - n0.extend(0.0) * w);
            vertices.push(p0 + n0.extend(0.0) * w);
            let mut p_cur = p1;
            loop {
                ...
                if let Some(p_next) = line.next() {
                    let v = (p_next - p_cur).truncate().normalize_or_zero();
                    flip = v.dot(dir) < 0.0;
                    let n = if flip {
                    ...
                    vertices.push(p_cur - n.extend(0.0) * w1);
                    vertices.push(p_cur + n.extend(0.0) * w1);
                    p_cur = p_next;
                } else {
                    let n = Self::vertical(dir);
                    vertices.push(p_cur - n.extend(0.0) * w);
                    vertices.push(p_cur + n.extend(0.0) * w);
                    break;
                }
            }
